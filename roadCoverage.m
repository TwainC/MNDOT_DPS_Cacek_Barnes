...
if wJ + wT > d 
    ...
    mCov = (d + R*vA/vV - wJ - wT)*(3*R + d*vV/vA + (wJ+wT)*vV/vA)/2 + (wM/2 - (d + R*vA/vV - wJ - wT))*(2*R + d*vV/vA);
    
elseif wJ + wT > d + R*vA/vV
    tCov = (d + R*vA/vV - wJ)*((R+wJ*vA/vV)+(2*R+d(vV/vA)))/2 + (2*R + d*vV/vA)*(wJ+wT-(d+R*vA/vV));
    mCov = wM/2 * (2*R + d*vV/vA);
    
else
    tCov = wT * (2*R + wJ*vV/vA + (wJ+wT)*vV/vA)/2;
    mCov = (d + R*vA/vV - (wJ + wT))*(3*R + (wJ+wT)*vV/vA + d*vV/vA)/2 + (wM/2 + wJ + wT - (d + R*vA/vV))*(2*R + d*vV/vA);
end
